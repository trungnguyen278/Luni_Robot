...
// Real CRC-8 (poly 0x07, init 0x00) — replaces the old XOR "checksum" (same
// change as UartProtocol). XOR missed byte reorders and cancelling bit-flips;
// CRC-8 catches both at the same 1-byte footer. Bitwise form so it is portable
// and byte-for-byte identical on both MCUs.
